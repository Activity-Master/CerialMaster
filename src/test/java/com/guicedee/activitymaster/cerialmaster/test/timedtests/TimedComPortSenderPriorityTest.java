...
public class TimedComPortSenderPriorityTest
{
  ...
  public void priorityRunsImmediatelyAfterCurrent_andPerMessageUniCompletes() throws Exception
  {
    ...
    try
    {
      conn = svc.getComPortConnectionDirect(20)
                 .await()
                 .indefinitely();
      conn.connect();
      conn.disconnect();
    ...
    TimedComPortSender sender = conn.getOrCreateTimedSender(new Config(0, 8, 250));

    // Track the order that messages start
    List<String> startOrder = new CopyOnWriteArrayList<>();
    var progressSub = sender.messageProgress()
                          .subscribe()
                          .withSubscriber(AssertSubscriber.create(256))
        ;
    ...
    var A = new MessageSpec("A", "PAYLOAD-A", new Config(0, 12, 300));
    var B = new MessageSpec("B", "PAYLOAD-B", new Config(0, 12, 300));
    var C = new MessageSpec("C", "PAYLOAD-C", new Config(0, 12, 300));
    var groupUni = sender.enqueueGroup(List.of(A, B, C));

    // Wait (bounded) for A to start
    for (int i = 0; i < 200 && !startOrder.contains("A"); i++)
    {
      Thread.sleep(10);
    }
    ...
    var P = new MessageSpec("P", "PAYLOAD-P", new Config(0, 8, 300));
    ...
    for (int i = 0; i < 200 && !startOrder.contains("P"); i++)
    {
      Thread.sleep(10);
    }
    ...
    for (int i = 0; i < 200 && !startOrder.contains("B"); i++)
    {
      Thread.sleep(10);
    }
    ...
    for (int i = 0; i < 200 && !startOrder.contains("C"); i++)
    {
      Thread.sleep(10);
    }
    System.out.println("[TEST] StartOrder after C wait: " + startOrder);
    var Q = new MessageSpec("Q", "PAYLOAD-Q", new Config(0, 8, 300));
    ...
    {
      if (!uniqueStartOrder.contains(id))
      {
        uniqueStartOrder.add(id);
      }
    }
    ...
    MessageResult pResult = pUni.await()
                                                   .indefinitely();
    ...
    MessageResult qResult = qUni.await()
                                                   .indefinitely();
